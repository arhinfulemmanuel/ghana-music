                    ...
                    @foreach ($posts as $post)
                        <div class="uk-card uk-card-default uk-card-hover uk-card-body uk-width-5-6@m uk-margin-bottom uk-margin-left uk-margin-right">
                            ...
                            <div class="uk-margin-top row">
                                <form action="{{route('delete_ad', ['post'=>$post->id])}}" method="post">
                                    @csrf
                                    <button type="submit" class="uk-margin-left uk-margin-right btn btn-danger">Delete</button>
                                </form>
                                <a href="/" class="btn btn-success uk-margin-left" style="color:white">Visit</a>
                            </div>
                        </div>
                    @endforeach
                @endif
